...
let animationSpeed = 45; // 한 프레임당 선 그려질 길이 (픽셀)
let drawingProgress = 0; // 현재 그려지는 선의 진행 정도

let categoryStyles = {
  "집":    { color: [80, 80, 80, 80],     weight: 5 },
  "남의집": { color: [0, 0, 255, 127],     weight: 6 },
  "학교":  { color: [255, 165, 0, 127],   weight: 10 },
  "꼼방":  { color: [255, 75, 0, 127],   weight: 8 },
  "식당":  { color: [0, 128, 0, 127],   weight: 9 },
  "기타":  { color: [255, 105, 180, 127], weight: 13 },
  "알바":  { color: [0, 255, 255, 127],   weight: 8 }
};
...
function drawLinesAnimated() {
  ...
  for (let date of sortedDates) {
    ...
    for (let i = 0; i < dayPoints.length; i++) {
      ...
      let y2 = p2 ? offsetY + p2.y * spacing : offsetY + rows * spacing;
